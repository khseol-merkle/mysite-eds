...
export function filterData(searchTerms, data, rootPath = '/') {
	const normalizedRoot = rootPath.endsWith('/') ? rootPath : `${rootPath}/`;
	const scoped = rootPath === '/'
		? data
		: data.filter((item) => item.path === rootPath || item.path.startsWith(normalizedRoot));
	...
	return [
		...foundInHeader.sort((a, b) => a.minIdx - b.minIdx),
...
async function handleSearch(e, block, config) {
	...
	const filteredData = filterData(searchTerms, data, config.rootPath);
...
export default async function decorate(block) {
	// extract root path from authored content (first cell text)
	const firstCell = block.querySelector('div > div');
	const rootPath = firstCell?.textContent?.trim() || '/';

	// extract source URL from authored link, or default
	const sourceLink = block.querySelector('a[href]');
	const source = sourceLink ? sourceLink.href : '/query-index.json';

	const placeholders = {};

	block.innerHTML = '';
	block.append(
		searchBox(block, { source, rootPath, placeholders }),
